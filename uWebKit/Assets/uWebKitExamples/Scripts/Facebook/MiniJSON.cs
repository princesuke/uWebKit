...
index += 4;
	...
	protected bool SerializeObjectOrArray (object objectOrArray, StringBuilder builder)
	{
		if (objectOrArray is Hashtable)
			return SerializeObject ((Hashtable)objectOrArray, builder); else if (objectOrArray is ArrayList)
	...
	protected bool SerializeValue (object value, StringBuilder builder)
	{
		...
		if (value == null)
			builder.Append ("null"); else if (value.GetType ().IsArray)
			SerializeArray (new ArrayList ((ICollection)value), builder); else if (value is string)
			SerializeString ((string)value, builder); else if (value is Char)
			SerializeString (Convert.ToString ((char)value), builder); else if (value is Hashtable)
			SerializeObject ((Hashtable)value, builder); else if (value is ArrayList)
			SerializeArray ((ArrayList)value, builder); else if ((value is Boolean) && ((Boolean)value == true))
			builder.Append ("true"); else if ((value is Boolean) && ((Boolean)value == false))
			builder.Append ("false"); else if (value.GetType ().IsPrimitive)
